...
mod tests {
    ...
    #[test]
    fn test_diag() {
        let v = RowVector::new([1, 2, 3]);
        let m = v.diag();
        assert_eq!(m, Matrix::new([[1, 0, 0], [0, 2, 0], [0, 0, 3]]));
    }

    #[test]
    fn test_shape() {
        let v = RowVector::new([1.0, 2.0, 3.0]);
        assert_eq!(v.shape(), 3);

        let empty_v = RowVector::<f64, 0>::new([]);
    ...
    #[test]
    fn test_view_mut() {
        let mut v = RowVector::new([1.0, 2.0, 3.0]);
        {
            let mut view = v.view_mut::<2>(1).unwrap();
            view[0] = 10.0;
            view[1] = 20.0;
        }
        assert_eq!(v, RowVector::new([1.0, 10.0, 20.0]));
    ...
    #[test]
    fn test_magnitude() {
        let v = RowVector::<f64, 3>::new([3.0, 4.0, 0.0]);
    ...
    fn test_from_vector_view() {
        ...
        assert_eq!(new_v, RowVector::new([2.0, 3.0]));

        let v = Vector::new([1.0, 2.0, 3.0]);
        let new_v: RowVector<f64, 3> = RowVector::from(v.t());
        assert_eq!(new_v, RowVector::new([1.0, 2.0, 3.0]));
    }
    ...
    fn test_from_matrix() {
        ...
        assert_eq!(v, RowVector::new([1.0, 2.0, 3.0]));
    }

    #[test]
    fn test_from_matrix_view() {
        let m = Matrix::new([[1.0, 2.0, 3.0], [3.0, 4.0, 5.0]]);
        let view = m.view::<1, 3>((1, 0)).unwrap();
        let v: RowVector<f64, 3> = RowVector::from(view);
        assert_eq!(v, RowVector::new([3.0, 4.0, 5.0]));
    }

    #[test]
    fn test_from_matrix_view_mut() {
        let mut m = Matrix::new([[1.0, 2.0, 3.0], [3.0, 4.0, 5.0]]);
